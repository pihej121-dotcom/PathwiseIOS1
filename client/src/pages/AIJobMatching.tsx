...
export default function AIJobMatching() {
  ...
  const { data: jobMatches = { jobs: [], totalCount: 0, page: 1, limit: 20 }, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Job search freshness parameters
      params.append("limit", "10");
      params.append("daysBack", "7");
      params.append("sortBy", "newest");
      params.append("page", String(Math.floor(Math.random() * 3) + 1));
      params.append("_timestamp", String(Date.now()));
      params.append("_random", String(Math.random()));
  ...
  return (
    <div className="space-y-6">
      <LoadingExperience isLoading={tailorResumeMutation.isPending} operation="jobs" />
      ...
      {jobMatches?.jobs && (
        <div className="space-y-4">
          {/* Removed “Page X of Y” section here */}
      ...
      <Dialog open={tailorModalOpen} onOpenChange={setTailorModalOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Resume Tailored for {selectedJob?.title}</DialogTitle>
          </DialogHeader>
          ...
          {tailoredResult && (
            <div className="space-y-6">
              ...
              {/* Remaining Gaps */}
              {tailoredResult.analysis?.remainingGaps &&
                tailoredResult.analysis.remainingGaps.length > 0 && (
                  <div>
              ...
              <div className="flex justify-end pt-4 border-t border-border">
                <Button variant="outline" onClick={() => setTailorModalOpen(false)}>
                  Close
                </Button>
              </div>
            </div>
